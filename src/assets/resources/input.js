...
var REORDER_BLOCK = exports.REORDER_BLOCK = 'REORDER_BLOCK';

/*
 * Block types
 */
var ADD_BLOCK_TYPE = exports.ADD_BLOCK_TYPE = 'ADD_BLOCK_TYPE';
var REMOVE_BLOCK_TYPE = exports.REMOVE_BLOCK_TYPE = 'REMOVE_BLOCK_TYPE';
var REORDER_BLOCK_TYPE = exports.REORDER_BLOCK_TYPE = 'REORDER_BLOCK_TYPE';
...
exports.copyObjectKeys = copyObjectKeys;
...
function copyObjectKeys() {
	var keys = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
	var from = arguments[1];

	return keys.reduce(function (to, k) {
		return Object.assign(to, _defineProperty({}, k, from[k]));
	}, {});
}

function resolveIndex(index, size) {
...
function addBlock(block) {
	var parentBlockId = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;

	block = Object.assign({ id: (0, _data.generateNewId)() }, block);

	return {
		type: _actions.ADD_BLOCK,
		payload: { block: block, parentBlockId: parentBlockId }
	};
}

function removeBlock(blockId) {
	return {
		type: _actions.REMOVE_BLOCK,
		payload: { blockId: blockId }
	};
}

function reorderBlock(blockId, index) {
	return {
		type: _actions.REORDER_BLOCK,
		payload: { blockId: blockId, index: index }
	};
}
...
exports.default = blockStructureReducer;
...
function blockStructureReducer() {
		...
		case _actions.ADD_BLOCK:
			{
				var _action$payload = action.payload,
				    _block = _action$payload.block,
				    parentBlockId = _action$payload.parentBlockId;

				// Non-strict inequality matches both null and undefined

				if (parentBlockId == null) {
					return [].concat(_toConsumableArray(state), [{ id: _block.id, level: 1 }]);
				}

				var parentItem = state.find(function (item) {
					return item.id === parentBlockId;
				});
				var parentIndex = state.indexOf(parentItem);

				if (parentItem && parentIndex > -1) {
					var nextItem = state.find(function (item, index) {
						return index > parentIndex && item.level <= parentItem.level;
					});
					var nextIndex = nextItem ? state.indexOf(nextItem) : -1;

					var newItem = { id: _block.id, level: parentItem.level + 1 };
...
function blocksReducer() {
	...
	switch (action.type) {
		case _actions.ADD_BLOCK:
			{
				var block = action.payload.block;
				...
				if (!(block.id in state)) {
					return Object.assign(_defineProperty({}, block.id, block), state);
				}
			}
			break;
		case _actions.REMOVE_BLOCK:
...
exports.getBlocksAsTree = undefined;
...
var getBlockStructure = function getBlockStructure(state) {
	return state.blockStructure;
};

var getBlocksAsTree = exports.getBlocksAsTree = (0, _reselect.createSelector)([getBlocks, getBlockStructure], function (blocks, blockStructure) {
	var tree = [];
	var parentStack = [];
	...
	try {
		for (var _iterator = blockStructure[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
			var item = _step.value;

			var block = blocks[item.id];
			var treeItem = { block: block, level: item.level, children: [] };
			var peekItem = void 0;
			...
			var branch = peekItem ? peekItem.children : tree;
...
var _blocks = __webpack_require__(11);

var _blocks2 = _interopRequireDefault(_blocks);

var _blockStructure = __webpack_require__(10);

var _blockStructure2 = _interopRequireDefault(_blockStructure);

var _blocks3 = __webpack_require__(9);

var _blocks4 = __webpack_require__(12);
...
var reducer = (0, _redux.combineReducers)({
	...
	blockStructure: _blockStructure2.default
});
...
store.dispatch((0, _blocks3.addBlock)({
	id: 4,
	blockType: 'bt1'
}));
...
console.log((0, _blocks4.getBlocksAsTree)(store.getState()));
