 ...
 * Neo input class
 */
Craft.NeoInput = Garnish.Base.extend(
{
	id: null,
	blockTypes: null,
	...
	init: function(id, blockTypes, inputNamePrefix, maxBlocks)
	{
		...
		var $blocks = this.$blockContainer.children(),
			collapsedBlocks = Craft.NeoInput.getCollapsedBlockIds();
		...
		for (var i = 0; i < $blocks.length; i++)
		{
			...
			var block = new NeoBlock(this, $block);
...
{
	collapsedBlockStorageKey: 'Craft-'+Craft.siteUid+'.NeoInput.collapsedBlocks',

	getCollapsedBlockIds: function()
	{
		if (typeof localStorage[Craft.NeoInput.collapsedBlockStorageKey] == 'string')
		{
			return Craft.filterArray(localStorage[Craft.NeoInput.collapsedBlockStorageKey].split(','));
		}
		else
		{
			return [];
	...
	{
		localStorage[Craft.NeoInput.collapsedBlockStorageKey] = ids.join(',');
	},
	...
	rememberCollapsedBlockId: function(id)
	{
		...
		{
			var collapsedBlocks = Craft.NeoInput.getCollapsedBlockIds();

			if ($.inArray(''+id, collapsedBlocks) == -1)
			{
				collapsedBlocks.push(id);
				Craft.NeoInput.setCollapsedBlockIds(collapsedBlocks);
			}
		}
	},
...
var NeoBlock = Garnish.Base.extend(
{
	neo: null,
	$container: null,
	$titlebar: null,
	...
	collapse: function(animate)
	{
		...
		if (!this.isNew)
		{
			Craft.NeoInput.rememberCollapsedBlockId(this.id);
		}
		else
		{
			if (!this.$collapsedInput)
			{
				this.$collapsedInput = $('<input type="hidden" name="'+this.neo.inputNamePrefix+'['+this.id+'][collapsed]" value="1"/>').appendTo(this.$container);
			}
			else
			{
	...
	expand: function()
	{
		...
		if (!this.isNew)
		{
			Craft.NeoInput.forgetCollapsedBlockId(this.id);
		}
	...
	{
		var batchAction = (this.neo.blockSelect.totalSelected > 1 && this.neo.blockSelect.isSelected(this.$container)),
			$option = $(option);
		...
		switch ($option.data('action'))
		{
			case 'collapse':
			{
				if (batchAction)
				{
					this.neo.collapseSelectedBlocks();
				}
				else
				{
					this.collapse(true);
			...
			case 'disable':
			{
				if (batchAction)
				{
					this.neo.disableSelectedBlocks();
				}
				else
				{
					this.disable();
			...
			{
				var type = $option.data('type');
				this.neo.addBlock(type, this.$container);
				break;
			}
	...
	{
		this.$container.velocity(this.neo.getHiddenBlockCss(this.$container), 'fast', $.proxy(function()
		{
			this.$container.remove();
			this.neo.updateAddBlockBtn();
		}, this));
	}
});
