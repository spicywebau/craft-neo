 ...
 * Matrix input class
 */
window.NeoInput = Garnish.Base.extend(
{
	id: null,
	blockTypes: null,
	...
	init: function(id, blockTypes, inputNamePrefix, maxBlocks)
	{
		...
		var $blocks = this.$blockContainer.children(),
			collapsedBlocks = window.NeoInput.getCollapsedBlockIds();
...
},
{
	...
	{
		if (typeof localStorage[window.NeoInput.collapsedBlockStorageKey] == 'string')
		{
			return Craft.filterArray(localStorage[window.NeoInput.collapsedBlockStorageKey].split(','));
		}
		else
		{
			return [];
	...
	{
		localStorage[window.NeoInput.collapsedBlockStorageKey] = ids.join(',');
	},
	...
	rememberCollapsedBlockId: function(id)
	{
		...
		{
			var collapsedBlocks = window.NeoInput.getCollapsedBlockIds();

			if ($.inArray(''+id, collapsedBlocks) == -1)
			{
				collapsedBlocks.push(id);
				window.NeoInput.setCollapsedBlockIds(collapsedBlocks);
			}
		}
	},
...
var MatrixBlock = Garnish.Base.extend(
{
	...
	collapse: function(animate)
	{
		...
		if (!this.isNew)
		{
			window.NeoInput.rememberCollapsedBlockId(this.id);
		}
		else
		{
	...
	expand: function()
	{
		...
		if (!this.isNew)
		{
			window.NeoInput.forgetCollapsedBlockId(this.id);
		}
