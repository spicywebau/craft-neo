...
/**
 * Neo configurator class
 */
Craft.NeoConfigurator = Garnish.Base.extend(
{
	fieldTypeInfo: null,
	...
	init: function(fieldTypeInfo, inputNamePrefix)
	{
		...
		this.$container = $('#'+this.inputIdPrefix+'-neo-configurator:first .input:first');
		...
		this.blockTypeSettingsModal.onSubmit = $.proxy(function(name, handle)
		{
			...
			var $item = $(
				'<div class="neoconfigitem mci-blocktype" data-id="'+id+'">' +
					...
					'</div>' +
					'<input class="hidden" name="types[Neo][blockTypes]['+id+'][name]">' +
					'<input class="hidden" name="types[Neo][blockTypes]['+id+'][handle]">' +
				'</div>'
